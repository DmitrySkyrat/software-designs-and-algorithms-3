...
describe('Magazine', () => {
  it('toString should return correct value', () => {
    ...
    for (const page of magazine) {
      expect(`${magazine.toString()}, ${page?.toString()}`).toEqual(`Magazine: G.Q with number of pages: 2, here is page with article #${counter} and it\'s material is glossy paper`);
      counter++;
    }
  });
});
